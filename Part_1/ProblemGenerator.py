...
def main():
    ...
    for x in range(0, 2 * options.drones) :
    ...
    person_location = setup_person_location(person, location[1:])
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        f.write("(:domain EmergencyServiceLogisticDomain)\n")
        f.write("(:objects\n")
        ...
        for i in range(0, len(person)) :
            for j in range(0, len(content_types)) :
                if need[i][j] :
                    f.write(f"(personNeeds {person[i]} {content_types[j]})\n")
                else :
        ...
        for armIndex in range(0, len(arm)):
            ...
            if (armIndex != 0 and armIndex % 2 == 0) :
                droneIndex += 1
        f.write("\n")
